...
class Transaction
{
    ...
    public function map(array $transactions, $template)
    {
        foreach ($transactions as $index => $transaction) {
            ...
            foreach ($this->getValue($transaction) as $subElement) {
                ...
                $this->$fieldName($subElement, $index, $transaction);
            }
        }
    ...
    {
        $this->transaction[$index]['transaction_type'][0]['transaction_type_code'] = $this->getTransactionType($this->attributes($subElement, 'code'));
    }
     ...
     * @param $transaction
     */
    protected function description($subElement, $index, $transaction)
    {
        $this->transaction[$index]['description'][0]['narrative'] = $this->groupNarrative($transaction['value'], 'description');
    }
