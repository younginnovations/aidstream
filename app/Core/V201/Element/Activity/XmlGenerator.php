...
class XmlGenerator
{
    ...
    public function generateXml(Activity $activity, Collection $transaction, Collection $result, Settings $settings, $activityElement, $orgElem, $organization)
    {
        ...
        $result            = $xml->save(config('xmlFiles.xml-file') . $publishedActivity);
    ...
    public function getMergeXml($published, $filename)
    {
        ...
        $iatiActivities->setAttribute('version', session('version'));
        ...
        foreach ($published as $xml) {
            ...
            $file   = sprintf("%s%s", config('xmlFiles.xml-file'), $xml);
            $addDom->load($file);
        ...
        $dom->saveXml();
        $dom->save(config('xmlFiles.xml-file') . $filename);
    }
