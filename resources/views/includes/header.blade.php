    ...
    <nav class="navbar navbar-default">
        ...
        <div class="navbar-collapse navbar-right">
            ...
            <div class="action-btn pull-left">
                @if(auth()->check())
                    <a href="{{ (auth()->user()->isSuperAdmin() || auth()->user()->isGroupAdmin() || auth()->user()->isDiAdmin()) ? url(config('app.super_admin_dashboard')) : (auth()->user()->getSystemVersion() == 2) ? url(config('app.admin_lite_dashboard')) : url(config('app.admin_dashboard')) }}"
